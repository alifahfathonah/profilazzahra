...
function perPageBindings () {
	...
	qodeOwlSlider();
	stockholmdQodeRecaptchaCallback();
...
function slideInNewPage(text, direction, direction2, animationTime, callbacks, url) {
	...
	newHTML.filter('script').each(function (i) {
		var evalString = '',
			itemID = this.id,
			itemIDValue = itemID.length ? itemID : '';
		
		if (this.text) {
			evalString = this.text;
		} else if (this.textContent) {
			evalString = this.textContent;
		} else if (this.innerHTML) {
			evalString = this.innerHTML;
		}
		
		if (itemIDValue.indexOf('variation-template') === -1) {
			$j.globalEval(evalString);
		}
	});

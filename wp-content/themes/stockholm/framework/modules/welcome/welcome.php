...
if ( ! class_exists( 'StockholmQodeWelcomePage' ) ) {
	class StockholmQodeWelcomePage {
		/**
		 * singleton class
		 */
		...
		/**
		 * get the instance of StockholmQodeWelcomePage
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_stockholm_qode_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_stockholm_qode_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_stockholm_qode_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'stockholm_qode_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'stockholm' ),
				esc_html__( 'About', 'stockholm' ),
				current_user_can( 'edit_theme_options' ),
				'stockholm_qode_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'stockholm_qode_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			stockholm_qode_get_module_template_part(  'templates/welcome','welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			if ( $hook === 'appearance_page_stockholm_qode_welcome_page' ) {
				wp_enqueue_style( 'stockholm-qode-welcome-page-style', QODE_FRAMEWORK_MODULES_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}
}

StockholmQodeWelcomePage::get_instance();